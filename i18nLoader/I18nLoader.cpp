...
I18nLoader::~I18nLoader()
{
    if (mData != nullptr)
    {
        uint8_t *data = (uint8_t *)mData;
        delete[] data;
        mData = nullptr;
    }
}
// 加载文件,检查版本是否一致
bool I18nLoader::load(const char *filename, const uint32_t version)
{
    if (mData != nullptr)
    {
        uint8_t *data = (uint8_t *)mData;
    ...
    // 关闭文件
    fclose(fp);
    mData = (FileHeader *)data;
    // 检查数据是否有效
    if (false == checkData(version,size))
    {
        delete[] data;
        mData = nullptr;
        return false;
...
// 通过枚举值翻译到字符串
const char *I18nLoader::translate(uint32_t id)
{
...
struct Crc32
{
    ...
    static uint32_t cal(uint32_t hash, const uint8_t *data, size_t len)
    {
        size_t i;
...
// 检查数据是否有效
bool I18nLoader::checkData(const uint32_t version, const size_t total) const
{
    // 检查魔数标识
    if (mData->mMagic[0] != 'i' ||
        mData->mMagic[1] != '1' ||
    ...
    uint32_t hash = Crc32::cal(0, data + 8, total - 8);
    // 比对文件hash
    if (hash != mData->mHash)
        return false;
    // 检查版本
    ...
    {
        mData->mTsls[i].mTsl = (TslItem *)(base + mData->mTsls[i].mOffset);
    }
